...
foreach my $params (@$encoding_parameters) {
    ...
    is(filesize($OUTPUT_FILENAME), $params->{output_filesize}, , "[$count] file size of output file");
...
sub mpeg_audio_info {
  ...
  return parse_mpeg_header($buffer);
}
